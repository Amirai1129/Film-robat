        ...
        if manual == False:
            ...
            try:
                if settings['auto_ffilter']:
                    ai_search = True
                    reply_msg = await message.reply_text(f"<b><i>درحال ارسال {message.text} 🔍</i></b>")
...
async def pm_text(bot, message):
    ...
    if PM_SEARCH == True:
        ai_search = True
        reply_msg = await bot.send_message(message.from_user.id, f"<b><i>درحال ارسال {content} 🔍</i></b>", reply_to_message_id=message.id)
...
async def next_page(bot, query):
   ...
   # if not search:
    ...
    if settings['button']:
        ...
        btn.insert(0, 
            [
                InlineKeyboardButton("ارسال همه", callback_data=f"sendfiles#{key}")
            ]
        )
        btn.insert(0, [
            InlineKeyboardButton(f'کیفیت ها', callback_data=f"qualities#{key}"),
            InlineKeyboardButton("کمپانی ها", callback_data=f"languages#{key}"),
            InlineKeyboardButton("بر اساس سال", callback_data=f"years#{key}")
        ])
    else:
        btn = []
        btn.insert(0, 
    ...
    try:
        if settings['max_btn']:
            ...
            if n_offset == 0:
                ...
                btn.append([InlineKeyboardButton("صفحه ها", callback_data="pages"), InlineKeyboardButton(f"{math.ceil(int(offset)/10)+1} / {math.ceil(total/10)}", callback_data="pages"), InlineKeyboardButton("𝐍𝐄𝐗𝐓 ➪", callback_data=f"next_{req}_{key}_{n_offset}")])
            else:
                btn.append(
                    [
            ...
            if n_offset == 0:
                ...
                btn.append([InlineKeyboardButton("صفحه ها", callback_data="pages"), InlineKeyboardButton(f"{math.ceil(int(offset)/int(MAX_B_TN))+1} / {math.ceil(total/int(MAX_B_TN))}", callback_data="pages"), InlineKeyboardButton("𝐍𝐄𝐗𝐓 ➪", callback_data=f"next_{req}_{key}_{n_offset}")])
            else:
                btn.append(
                    [
                        InlineKeyboardButton("⌫ 𝐁𝐀𝐂𝐊", callback_data=f"next_{req}_{key}_{off_set}"),
    ...
    if gl == False:
        ...
        if k == False:
            ...
            if files:
                ...
                ai_search = True
                reply_msg = await query.message.edit_text(f"<b><i>درحال ارسال {movie} 🔍</i></b>")
...
async def years_cb_handler(client: Client, query: CallbackQuery):
    ...
    try:
        if int(query.from_user.id) not in [query.message.reply_to_message.from_user.id, 0]:
            return await query.answer(
                f"⚠️ سلام {query.from_user.first_name},\nاین فایل توسط شخص دیگری درخواست داده شده است,\nلطفا خودتان درخواست کنید سپس قادر به دانلود خواهید بود",
                show_alert=True,
            )
    except:
        pass
    ...
    btn.insert(
        ...
        [
            InlineKeyboardButton(
                text="سال مورد نظر را انتخاب کنید", callback_data="ident"
            )
        ],
    )
    req = query.from_user.id
    offset = 0
    btn.append([InlineKeyboardButton(text="بازگشت به خانه", callback_data=f"fy#homepage#{key}")])
...
async def filter_yearss_cb_handler(client: Client, query: CallbackQuery):
    ...
    if not files:
        await query.answer("🚫 چیزی یافت نشد 🚫", show_alert=1)
        return
    temp.GETALL[key] = files
    settings = await get_settings(message.chat.id)
    pre = 'filep' if settings['file_secure'] else 'file'
    ...
    if offset != "":
        try:
            if settings['max_btn']:
                btn.append(
                    [InlineKeyboardButton("صفحه ها", callback_data="pages"), InlineKeyboardButton(text=f"1/{math.ceil(int(total_results)/10)}",callback_data="pages"), InlineKeyboardButton(text="𝐍𝐄𝐗𝐓 ➪",callback_data=f"next_{req}_{key}_{offset}")]
                )
    
            else:
                btn.append(
                    [InlineKeyboardButton("صفحه ها", callback_data="pages"), InlineKeyboardButton(text=f"1/{math.ceil(int(total_results)/int(MAX_B_TN))}",callback_data="pages"), InlineKeyboardButton(text="𝐍𝐄𝐗𝐓 ➪",callback_data=f"next_{req}_{key}_{offset}")]
                )
        except KeyError:
            await save_group_settings(query.message.chat.id, 'max_btn', True)
            btn.append(
        ...
        btn.append(
            [InlineKeyboardButton(text="صفحه دیگری وجود ندارد",callback_data="pages")]
        )
    if lang != "homepage":
        req = query.from_user.id
        offset = 0
...
async def episodes_cb_handler(client: Client, query: CallbackQuery):
    ...
    req = query.from_user.id
    offset = 0
    btn.append([InlineKeyboardButton(text="بازگشت به خانه", callback_data=f"fe#homepage#{key}")])
...
async def languages_cb_handler(client: Client, query: CallbackQuery):
    ...
    btn.insert(
        ...
        [
            InlineKeyboardButton(
                text="کمپانی مورد نظر را انتخاب کنید", callback_data="ident"
            )
        ],
    )
    req = query.from_user.id
    offset = 0
    btn.append([InlineKeyboardButton(text="بازگشت به خانه", callback_data=f"fl#homepage#{key}")])
...
async def seasons_cb_handler(client: Client, query: CallbackQuery):
    ...
    req = query.from_user.id
    offset = 0
    btn.append([InlineKeyboardButton(text="بازگشت به خانه", callback_data=f"next_{req}_{key}_{offset}")])
...
async def qualities_cb_handler(client: Client, query: CallbackQuery):
    ...
    btn.insert(
        ...
        [
            InlineKeyboardButton(
                text="کیفیت مورد نظر را انتخاب کنید", callback_data="ident"
            )
        ],
    )
    req = query.from_user.id
...
async def filter_qualities_cb_handler(client: Client, query: CallbackQuery):
    ...
    if settings["button"]:
        ...
        btn.insert(0, 
            [
                IInlineKeyboardButton("ارسال همه", callback_data=f"sendfiles#{key}")
            ]
        )
        btn.insert(0, [
            InlineKeyboardButton(f'کیفیت ها', callback_data=f"qualities#{key}"),
     ...
     #   if not search:
      ...
      #      return
        ...
        btn2 = [[
            ...
            InlineKeyboardButton("Rᴇᴏ̨ᴜᴇsᴛ Gʀᴏᴜᴘ Lɪɴᴋ", url="https://t.me/wcityirrr")
        ]]
        if query.from_user.id in ADMINS:
            user = await client.get_users(from_user)
            reply_markup = InlineKeyboardMarkup(btn)
        ...
        try:
            ...
            await log_msg.reply_text(
                ...
                quote=True,
                disable_web_page_preview=True,
                reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("🚀 لینک مستقیم 🚀", url=download),  # we download Link
                                                    InlineKeyboardButton('🖥️ پخش آنلاین 🖥️', url=stream)]])  # web stream Link
            )
            button = [[
                InlineKeyboardButton("🚀 لینک مستقیم 🚀", url=download),
                InlineKeyboardButton('🖥️ پخش آنلاین 🖥️', url=stream)
            ],[
                InlineKeyboardButton("• پخش در وب اپ تلگرام •", web_app=WebAppInfo(url=stream))
            ]]
            await query.message.reply_text(
                text="لینک دانلود مستقیم و پرسرعت شما ساخته شد. برای دانلود روی لینک مستقیم و برای پخش زنده روی پخش آنلاین کلیک کنید",
                quote=True,
                disable_web_page_preview=True,
            ...
            await query.answer(f"☣ مشکلی به وجود آمده است\n\n{e}", show_alert=True)
            return
    # don't change anything without contacting me @wcityirrr
        ...
        if PREMIUM_AND_REFERAL_MODE == True:
            buttons = [[
                ...
                InlineKeyboardButton('تقویم پی پر ویو ها', callback_data="shortlink_info")
            ],[
        ...
        buttons = [[
            InlineKeyboardButton('بازگشت', callback_data='filters')
        ]]
        await client.edit_message_media(
            query.message.chat.id, 
            query.message.id, 
        ...
        buttons = [[
             InlineKeyboardButton('دسترسی به آرشیو ها', callback_data='admin'),
         ], [ 
             InlineKeyboardButton('به زودی', callback_data='r_txt'),   
             InlineKeyboardButton('به زودی', callback_data='s_txt') 
         ], [ 
             InlineKeyboardButton('به زودی', callback_data='store_file'),   
             InlineKeyboardButton('به زودی', callback_data='tele') 
         ], [             
        ...
        buttons = [[
            InlineKeyboardButton('بازگشت', callback_data='manuelfilter')
        ]]
        await client.edit_message_media(
            query.message.chat.id, 
            query.message.id, 
        ...
        buttons = [[
            InlineKeyboardButton('بازگشت', callback_data='help'),
            InlineKeyboardButton('⟲ تلاش دوباره', callback_data='rfrsh')
        ]]
        await client.edit_message_media(
            query.message.chat.id, 
            query.message.id, 
        ...
        btn = [[
            InlineKeyboardButton("کمپانی مورد نظر را انتخاب کنید", callback_data="laninfo")
        ],[
            InlineKeyboardButton("WWE", callback_data="tamil_info"),
            InlineKeyboardButton("AEW", callback_data="english_info"),
            InlineKeyboardButton("TNA", callback_data="hindi_info")
        ],[
            InlineKeyboardButton("NJPW", callback_data="malayalam_info"),
            InlineKeyboardButton("GCW", callback_data="urdu_info"),
            InlineKeyboardButton("NOAH", callback_data="bangladesh_info")
        ],[
            InlineKeyboardButton("UFC", callback_data="telugu_info"),
            InlineKeyboardButton("ONE", callback_data="kannada_info"),
            InlineKeyboardButton("BELLATOR", callback_data="gujarati_info")
        ],[
            InlineKeyboardButton("بازگشت", callback_data="start")
        ]]
        await client.edit_message_media(
        ...
        btn = [[
            InlineKeyboardButton("بازگشت", callback_data="help"),
            InlineKeyboardButton("کانال پارس فایت", url="telegram.me/wcityirrr")
        ]]
        await client.edit_message_media(
            query.message.chat.id, 
            query.message.id, 
        ...
        btn = [[
            InlineKeyboardButton("بازگشت", callback_data="help"),
            InlineKeyboardButton("کانال پارس فایت", url="telegram.me/KingVj01")
        ]]
        await client.edit_message_media(
            query.message.chat.id, 
            query.message.id, 
...
async def auto_filter(client, name, msg, reply_msg, ai_search, spoll=False):
    ...
    if imdb:
        ...
        if settings["button"]:
            cap = f"<b>نام فایل درخواستی : {search}\n\nدرخواست توسط : {message.from_user.mention}\n\n⁉️ توجه کنید نام فایل درخواستی به انگلیسی نوشته بشه همراه سال یا روز پخش. \n\n⚠️ فایل ها تا 5 دقیقه دیگر حذف خواهند شد لطفا فایل موردنظر را جایی ارسال کرده سپس اقدام به دانلود کنید 🗑️\n\n</b>"
        else:
            cap = f"<b>نام فایل درخواستی : {search}\n\nدرخواست توسط : {message.from_user.mention}\n\n⁉️ توجه کنید نام فایل درخواستی به انگلیسی نوشته بشه همراه سال یا روز پخش. \n\n⚠️ فایل ها تا 5 دقیقه دیگر حذف خواهند شد لطفا فایل موردنظر را جایی ارسال کرده سپس اقدام به دانلود کنید 🗑️\n\n</b>"
...
async def advantage_spell_chok(client, name, msg, reply_msg, vj_search):
    ...
    if AI_SPELL_CHECK == True and vj_search == True:
        vj_search_new = False
        vj_ai_msg = await reply_msg.edit_text("<b><i>درحال جستجو برای نام صحیح فایل درخواستی</i></b>")
        movienamelist = []
        ...
        btn.append([InlineKeyboardButton(text="بستن این صفحه", callback_data=f'spol#{reqstr1}#close_spellcheck')])
